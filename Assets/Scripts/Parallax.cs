...
public class Parallax : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        playerStartPos = player.transform.position.x; // Get the starting position of the player
    }
    void Update()
    {
        float offset = (player.transform.position.x - playerStartPos) * speed; // Calculate the offset based on the player's position and speed
        // This line finds out how much to offset the texture by.
        // We do this by subtracting our starting position X position by our current X position
        // We then multiply the offset by the speed so that we can have different layers scroll at different speeds.
